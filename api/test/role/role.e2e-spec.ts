...
import { CreateRoleDto } from '@/modules/role/role.dto';
...
afterAll(async done => {
  await Role.deleteOne({ name: createData.name });
  await mongoose.disconnect(done);
});

describe(`${EResources.Role} List`, () => {
  ...
  it('can get list', () => {
    return request(APP_URL)
      ...
      .expect(200)
      .expect(({ body }) => {
        resourceListExpects(expect, body, EResources.Role);
      });
  });
});

describe(`${EResources.Role} Create`, () => {
  ...
  it('can create', () => {
    return request(APP_URL)
      ...
      .expect(201)
      .expect(({ body }) => {
        createExpect<IRole>(expect, body, EResources.Role);
        const output = { ...body } as IApiItem<IRole>;
...
describe(`${EResources.Role} Detail`, () => {
  it('cannot get detail if not authenticated', async () => {
    found = await Role.findOne({ name: createData.name });
  ...
  it('can get detail', () => {
    return request(APP_URL)
      ...
      .expect(200)
      .expect(({ body }) => {
        showExpect<IRole>(expect, body, EResources.Role);
        const output = { ...body } as IApiItem<IRole>;
        Object.keys(createData).map(key => {
          expect(output.data[key]).toEqual(createData[key]);
        });
...
describe(`${EResources.Role} Update`, () => {
  ...
  it('can update', async () => {
    ...
    return request(APP_URL)
      ...
      .expect(200)
      .expect(({ body }) => {
        updateExpect<IRole>(expect, body, EResources.Role);
        const output = { ...body } as IApiItem<IRole>;
        Object.keys(createData).map(key => {
...
describe(`${EResources.Role} Delete`, () => {
  ...
  it('can delete', async () => {
    return request(APP_URL)
      ...
      .expect(200)
      .expect(({ body }) => {
        deleteExpect<IRole>(expect, body, EResources.Role);
      });
  });
});
